      ...
      for (string str = streamReader2.ReadLine(); str != null; str = streamReader2.ReadLine())
      {
        if (!str.StartsWith(";") && !str.StartsWith("/"))
        {
          ...
          if (str.StartsWith("EXT_SCB_"))
          {
            ...
            {
              case "EXT_SCB_OMEGASATURN":
                _ext_scoreboard.Add((IScoreBoard) new OmegaSaturnSender(Convert.ToInt32(strArray[1].Substring(3)), _ext_scb_refresh_interval));
                continue;
              case "EXT_SCB_FAVERO":
              ...
              case "EXT_SCB_MTVISUAL":
                ...
                continue;
              case "EXT_SCB_VELOMAX":
                _ext_scoreboard.Add((IScoreBoard) new VelomaxSender(Convert.ToInt32(strArray[1].Substring(3)), _ext_scb_refresh_interval));
                continue;
              case "EXT_SCB_HORN":
